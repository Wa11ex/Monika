...
class MemoryManager:
    ...
    def seed_lore(self, chunks_path: str):
        ...
        from utils.tools import encrypt_data
        ids, docs, metas = [], [], []
        for c in chunks:
            ids.append(c["id"])
            docs.append(c["text"])
        ...
        if ids:
            # 先用明文 upsert（生成正确的 embedding），再替换为密文
            self.lore_collection.upsert(ids=ids, documents=docs, metadatas=metas)
            encrypted_docs = [encrypt_data(d) for d in docs]
            self.lore_collection.update(ids=ids, documents=encrypted_docs)
            log.info("Lore 种子写入完成，共 %d 块（已加密）", len(ids))
    ...
    def retrieve_lore(self, query: str, n: int = 1, threshold: float = 0.45) -> list:
        ...
        try:
            ...
            from utils.tools import decrypt_data
            hits = []
            ...
            for i, (doc, dist) in enumerate(zip(results["documents"][0], results["distances"][0])):
                if dist < threshold:
                    ...
                    # 尝试解密；失败则兼容旧明文
                    plain = decrypt_data(doc)
                    hits.append({"id": hit_id, "text": plain if plain else doc, "distance": dist})
            return hits
        except Exception as e:
