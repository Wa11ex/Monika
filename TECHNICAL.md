...
| **Pub/Sub** | `core/event/EventBus.py` | `SpeechEvents`（SENTENCE / LIPSYNC_FRAME / TTS_DONE）解耦 UI 与语音管线 |
...
Bus._audio_player_worker 播放开始
  → events.emit(SpeechEvents.SENTENCE, sentence=text, is_first=False)
    → main_window._on_sentence()   （经 EventBus 订阅）
