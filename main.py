...
from core.po_manager import PoStateMachine
...
async def init_monika(log_callback=None):
    ...
    async def _load_ears_chain():
        '''SpeakerIdentifier -- Ears'''
        sp_id = None
        if get_config("speaker_id.enabled", False):
            try:
    ...
    async def _load_brain_chain():
        ...
        _log(">>> [Main] 正在加载语言模型...")
        _brain = await asyncio.to_thread(lambda: _create_brain(memory_manager=mem))
    ...
    (ears, speaker_identifier), (brain, memory), _ = await asyncio.gather(
        _load_ears_chain(),
        _load_brain_chain(),
        _await_tts(tts_task),
    )#齐头并进
