...
REM -- Detect CUDA, install torch + torchaudio + llama-cpp-python --
REM    必须在 requirements.txt 之前装：sentence-transformers / chromadb
REM    会间接依赖 torch，先装好对的版本，pip 后面才会跳过。
echo [*] Detecting GPU for torch / llama-cpp-python...
nvidia-smi >nul 2>&1
if errorlevel 1 (
    echo [INFO] No NVIDIA GPU. Installing CPU-only torch + llama-cpp-python.
    ...
    echo [*] Installing torch + torchaudio (!CUDA_TAG!)...
    pip install torch torchaudio --index-url https://download.pytorch.org/whl/!CUDA_TAG! --quiet 2>nul
    if errorlevel 1 (
        echo [WARN] PyTorch wheel for !CUDA_TAG! not found, trying cu124...
        pip install torch torchaudio --index-url https://download.pytorch.org/whl/cu124 --quiet 2>nul
        if errorlevel 1 (
            echo [WARN] PyTorch cu124 unavailable, falling back to CPU torch.
            pip install torch torchaudio --quiet
        )
    )

    echo [*] Installing llama-cpp-python (!CUDA_TAG!)...
    pip install llama-cpp-python --extra-index-url https://abetlen.github.io/llama-cpp-python/whl/!CUDA_TAG! --quiet 2>nul
    if errorlevel 1 (
        echo [WARN] llama wheel for !CUDA_TAG! not found, trying cu124...
        pip install llama-cpp-python --extra-index-url https://abetlen.github.io/llama-cpp-python/whl/cu124 --quiet 2>nul
        if errorlevel 1 (
            echo [WARN] llama pre-built wheel unavailable, source build (needs VS Build Tools)...
            echo        https://visualstudio.microsoft.com/visual-cpp-build-tools/
            pip install llama-cpp-python
        )
    )
)
echo [OK] torch + llama-cpp-python installed
echo.
...
echo     (funasr, chromadb, PyQt6, sentence-transformers - may take 10-30 minutes)
echo.
