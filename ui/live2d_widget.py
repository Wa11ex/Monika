...
class Live2DWidget(QWebEngineView):
    def __init__(self, parent=None):
        ...
        self._page_loaded = False         # HTML 页面是否已加载完成
        self._pending_model_url = None    # 页面加载完成后待执行的模型 URL

        # 页面加载完成后：标记就绪并执行挂起的 loadModel（必须早于 load() 连接）
    ...
    def load_model(self, model_path_or_name: str):
        ...
        # 便携版：代码在 src/ 子目录时，assets 在 src 的上一级
        ...
        # 页面已加载完成 -> 立即执行；否则由 _on_page_loaded 在 loadFinished 后执行
        if self._page_loaded:
            self._execute_load()
        else:
            logger.info("[Live2D] 页面尚未加载完成，等待 loadFinished 后再调用 loadModel")

    def _execute_load(self):
        '''真正调用 loadModel JS（必须在页面加载完成后执行）'''
        model_url = self._pending_model_url
        if not model_url:
            return
        self._pending_model_url = None
        
        logger.debug("[Live2D] 执行 loadModel")
    ...
    def _on_page_loaded(self, ok: bool):
        '''页面加载完成回调：标记就绪并执行待处理的模型加载'''
        self._page_loaded = True
        if self._pending_model_url:
            self._execute_load()

    async def wait_page_loaded(self, timeout: float = 5.0) -> bool:
        '''等待 HTML 页面加载完成（若已加载则立即返回 True）'''
        import asyncio
        if self._page_loaded:
            return True
        done = asyncio.Event()
        self.loadFinished.connect(lambda _ok: done.set())
        try:
