...
== 已有命令 ==
    \ck --sd   切换 AIUEO 口型调试面板
    \ck --tr   切换 Trait 数值调试面板
    \tool --ls     列出已注册的工具
    \tool --test     测试工具调用（read_file('README.md')）
    \tool --pwd      显示当前工作目录（工具只能访问此目录下的文件）
    \b           输出 Benchmark 报告
    \b --json    输出 Benchmark JSON 原始数据
'''
...
def _cmd_check_trait(win: "MainWindow") -> None:
    '''\ck --tr'''
    win._trait_panel.setVisible(not win._trait_panel.isVisible())
...
def _cmd_vision_cam(win: "MainWindow") -> None:
    '''\ck --cam：开关摄像头实时监视'''
    win.toggle_vision_monitor("camera")


def _cmd_vision_ss(win: "MainWindow") -> None:
    '''\ck --ss：开关屏幕实时监视'''
    win.toggle_vision_monitor("screen")


def _cmd_vision_both(win: "MainWindow") -> None:
    '''\ck --v：开关双流（摄像头 + 屏幕）并排监视'''
    win.toggle_vision_monitor("both")


def _cmd_ck_output(win: "MainWindow") -> None:
    '''\ck --output：开关模型原始 token 输出流面板'''
    win._ck_panel.setVisible(not win._ck_panel.isVisible())


COMMANDS: dict[str, callable] = {
    "ck --sd": _cmd_check_sound,
    "ck --tr": _cmd_check_trait,
    "ck --cam": _cmd_vision_cam,
    "ck --ss": _cmd_vision_ss,
    "ck --v": _cmd_vision_both,
    "ck --output": _cmd_ck_output,
    "tool --ls": _cmd_tool_list,
    "tool --test": _cmd_tool_test,
    "tool --pwd": _cmd_tool_pwd,
    "b": _cmd_bench_report,
    "b --json": _cmd_bench_json,
}
