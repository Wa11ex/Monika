...
_THINK_UNCLOSED_RE = re.compile(r'<think>.*', re.DOTALL)  # 未闭合的 think（流截断时）
_CS_DELIMITER = "</think>"
...
def _strip_think_blocks(text: str) -> str:
    ...
    也处理未闭合的 <think>（流截断时 think 没写完 </think> 就 EOS），
    防止 think 内容残留进 history 污染下一轮。
    '''
    text = _THINK_RE.sub('', text)               # 先删闭合的
    text = _THINK_UNCLOSED_RE.sub('', text)       # 再删未闭合的
    return text.lstrip('\n').strip()
...
class BrainBase(BaseBrain):
    ...
    # -- TTS 背压事件 --
    _tts_ready = None  # asyncio.Event, 由 SpeechEvents.TTS_DONE 事件 set
    ...
    def _init_shared(self, memory_manager=None):
        ...
        from core.event import TraitManager
        self.trait_manager = TraitManager(get_config("memory.traits.path", "assets/memory/traits.json"))
        self.trait_manager.load()
...
'''
    ...
    def _load_session(self):
        ...
        try:
            with open(self._session_path, "r", encoding="utf-8") as f:
                raw = f.read()
            # 尝试解密；失败则兼容旧明文格式
            from utils.tools import decrypt_json
            saved = decrypt_json(raw)
            if saved is None:
    ...
    def _save_session(self):
        ...
        try:
            ...
            from utils.tools import encrypt_json
            with open(self._session_path, "w", encoding="utf-8") as f:
                f.write(encrypt_json(clean_turns))
        except Exception as e:
    ...
    def _prepare_messages(self, text: str, speaker_info=None):
        ...
        # -- Trait 注入 ----------
        trait_hints = self.trait_manager.get_prompt_hints()
        if trait_hints and messages and messages[0]['role'] == 'system':
            messages[0] = {
                'role': 'system',
                'content': messages[0]['content'] + f"\n{trait_hints}",
            }
