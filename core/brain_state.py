...
class BrainStateMachine:
    ...
    async def process(
        ...
        # THINK 状态结束时暴露残留内容（供 brain_loader 续写）
        self.think_remnant = ""
        if self._state == S_THINK and self._state_buf.strip():
            self._think_parts.append(self._state_buf)
            self._state_buf = ""
            self.think_remnant = ''.join(self._think_parts)
            log.warning("[Brain.State] [WARN] 流在 THINK 状态结束（可能提前 <|im_end|>），"
                        "think 长度=%d chars", len(self.think_remnant))
        if self._think_parts:
            log.debug("[Brain.Think]\n%s", ''.join(self._think_parts)[:500])

    async def flush_remaining(self) -> Optional[str]:
        '''返回收尾时残留的 TALK 文本'''
        # 如果在 CODE 状态结束，先 pop 回 TALK（代码块未闭合，丢弃代码内容）
        if self._state == S_CODE and len(self._stack) > 1:
            self._pop()
            log.debug("[Brain.State] CODE 未闭合，pop 回 %s", S_NAME[self._state])
