...
import os
import time
from core.clients.vts_client import VTSManager
...
class SpeechEvents:
    '''MonikaBus 语音管线对外发布的事件名（配合 core.event.EventBus 使用）

    这里是事件契约的"权威来源"：语音管线负责 emit，UI / Brain 负责 subscribe，
    避免魔法字符串散落各处。emit 调用散落在 MonikaBus 内部，即使用范例。
    '''
    # 逐句显示：payload(sentence: str, is_first: bool=False)
    # is_first 保留供未来区分句块首句；当前 UI 用闭包状态管理，不消费该字段
    SENTENCE = "speech:sentence"

    # 口型帧：payload(open_y: float, scores: np.ndarray)
    LIPSYNC_FRAME = "lipsync:frame"

    # TTS 合成完成（背压）：无 payload
    TTS_DONE = "tts:done"


class MonikaBus:
    def __init__(self, vts_manager: VTSManager, tts: BaseTTS = None):
        from core.clients.tts_client import GPTSoVITSTTS
    ...
    async def _text_worker(self):
        ...
        while True:
            ...
            self.events.emit(SpeechEvents.TTS_DONE)
    ...
    async def _audio_player_worker(self):
        ...
        async def _play_one(wav_bytes, emotions, motions, text):
            # -- 逐句事件：音频开始播放时通知 UI 显示 --
            if text:
                self.events.emit(SpeechEvents.SENTENCE, sentence=text, is_first=False)
            for emo in emotions:
    ...
    async def _drive_lipsync(
        ...
        for i in range(len(times)):
            ...
            if now - _last_push >= _update_s:
                ...
                _last_push = now
                self.events.emit(SpeechEvents.LIPSYNC_FRAME, open_y=oy, scores=scores[i])

        # 播放结束，嘴归零
        self._l2d('set_mouth_params', _lipsync_mod.MOUTH_PARAMS_RESET)
        self.events.emit(
            SpeechEvents.LIPSYNC_FRAME,
            open_y=0.0,
